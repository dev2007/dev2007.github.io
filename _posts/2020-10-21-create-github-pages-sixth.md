...
只要项目是属于登录的GitHub账号的，vercel就会开始同步项目，中间可能还会提示为项目安装vercel插件，同意即可。如果导入时提示无权限，如下图所示，按提示进入GitHub设置项，为相应开启Vercel访问权限即可。
...
项目导入成功后，自动识别出项目为Jekyll并自动配置编译项，什么都不用动，直接点击“Deploy”进行部署即可。
...
我们使用vercel的目的，就是想通过它进行网站访问加速，所以此处，我们可以将上一篇配置的自定义域名解析到Vercel上来。

部署成功后点击“Open Dashboard”或者回到网站首页，进入项目详情后，点击项目的“Settings”进入设置页面。
...
> 需要注意的一点时，当把自定义域名指向Vercel的域名后，按[用GitHub Pages搭建博客（五）](/create-github-pages-fifth)中，对GitHub仓库中设置的域名配置将失效，进入Pages的设置中也会看到相应提示，如果有洁癖的话，可以按提示移除配置，不移除也没有影响。


*请看下一篇：[用GitHub Pages搭建博客（六）](/create-github-pages-seventh)*
